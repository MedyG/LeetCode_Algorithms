...
/* 一维数组转换为二叉树并返回根节点 */
template<typename T>
TreeNode* buildBinaryTree(vector<T>& input) {
	queue<TreeNode*> q;
	int index = 0;
...
template <typename T>
void print2DVec(vector<vector<T>>& result) {
	cout << "{" << endl;
	for (vector<vector<T>>::iterator it = result.begin(); it != result.end(); ++it) {
		for (vector<T>::iterator i = it->begin(); i != it->end(); ++i) {
			if (i == it->begin()) {
				cout << "  [";
			}
			if (i == it->end() - 1) {
...
cout << solution->longestValidParentheses(s) << endl;*/
	...
	//int a = (0 - 2147483648);
	//cout << a  << INT_MIN << endl;
	TreeNode* _root = buildBinaryTree(input);
